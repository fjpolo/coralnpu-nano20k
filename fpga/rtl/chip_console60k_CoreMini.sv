...
module chip_console60k_CoreMini(
 ...
 // =========================================================================
 // --- GLOBAL ENABLE SIGNALS (LUT logic) -----------------------------------
 // =========================================================================
 ...
 // NOTE: Wires from the CoreMini instantiation that were missing declarations 
 // are added here.
 logic tied_0_1b;
 logic [3:0] tied_0_4b;
 logic [4:0] tied_0_5b;
 ...
 logic [31:0] csr_in_value_0      /* synthesis syn_keep=1 */;
 logic [127:0] ibus_rdata         /* synthesis syn_keep=1 */;
 logic ibus_fault_valid          /* synthesis syn_keep=1 */;
 logic [31:0] ibus_fault_bits_epc /* synthesis syn_keep=1 */;
 logic [127:0] dbus_rdata         /* synthesis syn_keep=1 */;
 logic ebus_dbus_ready           /* synthesis syn_keep=1 */;
 logic ebus_fault_valid          /* synthesis syn_keep=1 */;
 logic ebus_fault_bits_write     /* synthesis syn_keep=1 */;
 logic [31:0] ebus_fault_bits_addr /* synthesis syn_keep=1 */;
 logic [31:0] ebus_fault_bits_epc  /* synthesis syn_keep=1 */;
 logic core_irq                  /* synthesis syn_keep=1 */;
 
 // OUTPUT WIRES (Consumed by o_pmod1 or tied off)
 // FIX: Added missing declarations for core outputs
 ...
 // Tie off unused UART output
 assign uart_tx_o = 2'b00;
...
`ifdef FULL_COREMINI
 /* synthesis syn_keep=1 */ SCore i_CoreMini_SCore(
    ...
    .io_ebus_dbus_rdata(tied_0_128b),
    ...
    // DEBUG PORTS (Tied off using the standard Verilog method of not connecting)
    .io_slog_valid(),
    .io_slog_addr(),
    .io_slog_data(),
    .io_debug_en(),
 ...
 // Bit 0: Combined check on all CSR outputs
 assign o_pmod1[0] = |{csr_out_value_0, csr_out_value_1, csr_out_value_2, csr_out_value_3};
 // Bit 1: Combined check on remaining CSR outputs
 assign o_pmod1[1] = |{csr_out_value_4, csr_out_value_5, csr_out_value_6, csr_out_value_7};
 // Bit 2: Core status (Halted, Fault, WFI)
 assign o_pmod1[2] = core_halted | core_fault | core_wfi;
 // Bit 3: Instruction Bus Status (Valid/Address)
 assign o_pmod1[3] = ibus_valid | (|ibus_addr[7:0]);
 // Bit 4: Data Bus Status (Valid/Write)
 assign o_pmod1[4] = dbus_valid | dbus_write;
 // Bit 5: Data Bus Address (LSB)
 assign o_pmod1[5] = |dbus_addr[7:0];
 // Bit 6: Execution Bus Valid/Write
 assign o_pmod1[6] = ebus_dbus_valid | ebus_dbus_write;
 // Bit 7: Execution Bus Address (LSB)
 assign o_pmod1[7] = |ebus_dbus_addr[7:0];
